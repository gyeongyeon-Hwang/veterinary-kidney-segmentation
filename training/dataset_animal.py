...
def check_for_nan(tensor, operation_name):
    if torch.isnan(tensor).any():
        print(f"NaN values detected after the {operation_name} operation.")
        exit(1)
...
class AnimalDataset(Dataset):
    def __init__(self, args, mode='train', k_fold=5, k=0, seed=0):
        ...
        print(img_name_list)
        print('Start loading %s dataset'%self.mode)
        ...
        for name in img_name_list:
            ...
            self.spacing_list.append(spacing)  # itk axis order is inverse of numpy axis order
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            if np.random.random() < 0.5:
                ...
                tensor_img, tensor_lab = augmentation.crop_3d(tensor_img, tensor_lab, [d + 15, h + 65, w + 65],
                                                              mode='random')
                ...
                tensor_img, tensor_lab = augmentation.crop_3d(tensor_img, tensor_lab, self.args.training_size,
                                                              mode='center')
            # if np.random.random() < 0.15:
            #     std = np.random.random() * 0.1
            #     tensor_img = augmentation.gaussian_noise(tensor_img, std=std)
            # if np.random.random() < 0.15:
            #     tensor_img = augmentation.brightness_multiply(tensor_img, multiply_range=[0.8, 1.2])
        ...
        check_for_nan(tensor_img, 'squeeze')

        assert tensor_img.shape == tensor_lab.shape
        if self.mode == 'train':
            return tensor_img, tensor_lab
        else:
            return tensor_img, tensor_lab, np.array(self.spacing_list[idx]), self.case_list[idx]
