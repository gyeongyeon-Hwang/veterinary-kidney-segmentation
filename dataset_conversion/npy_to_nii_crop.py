...
def main(hdf_path, save_directory):
    ...
    tmp_list = ['2876', '5864', '5921', '5923', '5984', '6039', '6074', '202100843', '16649', '16064', '16154', '13012', '13165', '13678', '13330', '13012',
                ...
                '9664', '202001130', '202001251', '202002464', '13438', '6350', '202100919', '202001395', '3190', '6127', '6237', '13367', '15229']
    ...
    with tqdm(total=len(file_path) - 1, ascii=True, desc=f'extract', dynamic_ncols=True) as pbar:
        ...
        for hdf in file_path:
            ...
            for image_idx in range(num_image):
                ...
                if np.min(np.unique(data)) < -3500.0 or np.max(np.unique(data)) > 4000.0:
                    continue
                ...
                print(f'data HU value: min->{np.min(np.unique(data))} | max->{np.max(np.unique(data))}')

                # if os.path.isdir(os.path.join(save_directory, f'segmentation/{contrast}/input/{case_id}.npy')):
                #     continue
                ...
                if case_id not in tmp_list:
                    ...
                    print(f'data shape : {data.shape} | depth:{depth}, height:{height}, width:{width}')
                    if height != 512 or width != 512:
                        continue
                    # if contrast == 'POST':
                    #     data = np.reshape(data, (height, depth, width, int(data.shape[0] / (depth * height * width))))
                    # else:
                    ...
                    if 2 in np.unique(label):
                        ...
                        #     label_record[1] = np.unique(label)
                    ...
                    #     label, label_record = convert_label(label, contrast)
                        ...
                        for i in range(depth):
                            label_img = label[:, :, i]
                            if 2 <= len(np.unique(label_img)) <= 8:
                                depth_range.append(i)
                        '''clip'''
                        # data, label = save(data, label, contrast, case_id, depth_range)
...
# def save(data, label, contrast, case_id, depth):
#     centercrop = torchvision.transforms.CenterCrop(size=(512, 512))
#     offset = 30
#     # print(max_l, min_l)
#     if len(depth) != 0:
#         max_l = max(depth)
...
#         vol_img = []
#         vol_label = []
#         # if min_l+max_l > 0 and min_l > 15:
#         if min_l + max_l > offset and min_l - offset > 0 and max_l + offset < data.shape[2]:
#             for i in range(min_l-offset, max_l+1+offset):
#                 # data_img, label_img = remove_noise(data[:, :, i], label[:, :, i], contrast, case_id, i)
#
#                 data_img, label_img = centercrop(torch.from_numpy(data[:, :, i])), centercrop(
#                     torch.from_numpy(label[:, :, i]))
#                 # print(f'crop size:{data_img.shape, label_img.shape}')
#                 vol_img.append(data_img.numpy())
#                 vol_label.append(label_img.numpy())
#         else:
#             for i in range(min_l, max_l+1):
#                 # data_img, label_img = remove_noise(data[:, :, i], label[:, :, i], contrast, case_id, i)
#                 # data_img, label_img = data[:, :, i], label[:, :, i]
#                 data_img, label_img = centercrop(torch.from_numpy(data[:, :, i])), centercrop(
#                     torch.from_numpy(label[:, :, i]))
#                 # print(f'crop size:{data_img.shape, label_img.shape}')
#                 vol_img.append(data_img.numpy())
#                 vol_label.append(label_img.numpy())
#
#         vol_data = np.stack(vol_img, axis=2)
#         vol_label = np.stack(vol_label, axis=2)
#         print(f'label unique: {np.unique(vol_label)}')
#         return vol_data, vol_label
#
#     else:
#         vol_data, vol_label = None, None
#         return vol_data, vol_label
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hdf_path', type=str, default='/hdd1/yeon_data/animal/hdf_file/', help='hdf file path')
    parser.add_argument('--save_path', type=str, default='./dataset/nii_raw_0829/',
                        help='save png file path')

    args = parser.parse_args()
    proc = multiprocessing.Process(target=main, args=(args.hdf_path, args.save_path))
    proc.start()
    proc.join()
